    ...
    async function handleLoad(_event: Event): Promise<void> {
        ...
        async function communicate(_url: RequestInfo): Promise<void> {
            ...
            for (let key of Object.keys(json)) {
                let value: FormDataEntryValue = json[key];
            ...
            function editItem(_event: Event): void {
                ...
                let identifyer: number = parseInt(triggerNumber);
                let list: HTMLElement = document.getElementById("lister" + identifyer);
                list.setAttribute("contenteditable", "true");
                let saveItem = function (_event: Event): void | null {
                    let trigger: string = (_event.target as HTMLButtonElement).id;
                    let triggerNumber: string = trigger.replace(/\D/g, "");
                    let identifyer: number = parseInt(triggerNumber);
                    let list: HTMLElement = document.getElementById("lister" + identifyer);
                    list.setAttribute("contenteditable", "false");
                    let saveButton: HTMLElement = document.getElementById("save" + identifyer);
